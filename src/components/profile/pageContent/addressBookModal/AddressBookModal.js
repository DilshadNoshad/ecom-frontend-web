...
const AddressBookModal = (props) => {
  ...
  //sendRequest();
  const { sendRequest, status, data: cities, error } = useHttp(getCityList, true);
...
//console.log('-----------address modal bf effect----------------');
  ...
  const optCityList = (cities)=> {
    ...
    const citylst = cities.map((obj)=> {
      return {'label': `${obj.cityName} - (${obj.cityCode})`, 
            'name': obj.cityName, 
            'code': obj.cityCode, 
            'value': obj.cityId, 
            'cityId': obj.cityId, 
  ...
  return (
    <Modal onClose={props.onClick}>
      ...
      <CheckoutForm cityList={cityList} address={props?.address} updateAddress={onAddressUpdate} onClose={props.onClick} />
    </Modal>
  );
};
